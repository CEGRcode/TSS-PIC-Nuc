...
# ───────────────────────────────────────────────
input_file = 'TF_M1_nearestTSS.csv'
...
output_dir = "TFBS_phase_barplots"
...
# ───────────────────────────────────────────────
# PART 3: TFBS Strand Bias Ratio (same / oppo)
# ───────────────────────────────────────────────
plt.rcParams.update({
    'axes.titlesize': 16,
    'axes.labelsize': 14,
...
# Calculate ratio (same / oppo), avoid division by zero
strand_counts['strand_ratio'] = strand_counts['same'] / strand_counts['oppo'].replace(0, 1e-6)

# Sort by ratio descending
strand_counts = strand_counts.sort_values(by='strand_ratio', ascending=False)

# Plot barplot of ratio
plt.figure(figsize=(14, 6))
sns.barplot(data=strand_counts, x='TFBS', y='strand_ratio', color='steelblue', width=BAR_WIDTH)
plt.axhline(1, color='gray', linestyle='--', linewidth=1)
plt.title('TFBS Strand Bias Ratio (same / oppo)')
plt.xlabel('TFBS')
plt.ylabel('Strand Ratio (same / oppo)')
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig('TFBS_strand_bias_ratio_only_barplot.png')
plt.close()

print("✅ Strand ratio bar plot saved as: TFBS_strand_bias_ratio_only_barplot.png")
...
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig('TFBS_phase_skew_barplot_same.png')
plt.close()
...
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig('TFBS_phase_skew_barplot_oppo.png')
plt.close()
...
skew_df[['TFBS', 'Strand', 'Max_Phase_Ratio', 'Best_Window']].to_csv(
    'TFBS_phase_skew_ratios.tsv', sep='\t', index=False
)

print("📄 Saved ratio values:")
print("   → TFBS_strand_bias_ratios.tsv (Part 5)")
print("   → TFBS_phase_skew_ratios.tsv (Part 6)")
